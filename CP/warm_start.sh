...
if [ ! -s "$directory/CP/gen_matrix.out" ]; then
  g++ $directory/CP/gen_matrix.c -o $directory/CP/gen_matrix.out
fi
matrix=$($directory/CP/gen_matrix.out $n)
touch $directory/CP/data.dzn
echo "n=$n;simplified_mode=true;warm_start=$matrix;" > $directory/CP/data.dzn
sol=$(minizinc --disable-warnings --statistics --json-stream --param-file $s  --solver-time-limit 300000 $directory/CP/warm_start.mzn $directory/CP/data.dzn)
if [ "$(echo "$sol" | jq -r 'select(.type == "status") | .status')" == "UNSATISFIABLE" ]; then
  ...
  json=$(minizinc --disable-warnings --statistics --json-stream --param-file $s --solver-time-limit $remaining $directory/CP/warm_start.mzn $directory/CP/data.dzn);
